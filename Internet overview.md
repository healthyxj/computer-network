...
分组以先到先服务的方式传输。传输时延可以说是发射时延。是将**所有分组的比特发送到链路所需要的时间**。L表示分组的长度，R表示链路传输速率,链路带宽。主要靠传输时延。traffic intensity = L / R,这个值如果接近1，时延就会很严重
...
吞吐量是发送方到接收方的数据传输速率。分为即时和平均的，平均的要看较小的。平均吞吐量是一般是取两者中较小的那个。
